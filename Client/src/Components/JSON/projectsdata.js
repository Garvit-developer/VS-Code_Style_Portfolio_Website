// // Replace below project details with your project details. Add all your projects details here as per below object structure, and it will populate it in projects sections.
// export const projectsdata = () => [
//     {
//         title: "IT solutions and software devlopment Website",
//         name: "Geek Theory Website",
//         banner: "/Geek_banner2.png",
//         detailBanner: "/Geek_banner.png",
//         skills: [
//             "CSS3",
//             "JavaScript",
//             "Node.js",
//             "React.js",
//             "Express",
//             "MongoDB",
//             "Tailwind CSS",
//             "REST APIs",
//         ],
//         description: `GeekTheo is an IT solutions company that delivers modern, scalable, and performance-driven digital services. The company specializes in web development, application design, and custom business solutions for clients. GeekTheo focuses on building secure, reliable, and user-focused systems using latest technologies. The website represents the company’s services, portfolio, and digital identity.`,
//         //(Optional) Live link of your project, if hosted
//         link: "geektheo.com",
//         github: null,
//         // Logo Image of your project
//         logo: "/GeekTheoryIcon.PNG",
//         features: [
//             "Upload and share any file of size upto 1GB of any format like text file, images, videos, application, pdf, excel, ppt and more.",
//             "Other important sections like User Profile, Tracking of number of files shared, amount of data transported, authentication and more.",
//         ],
//         // Screenshots of your project
//         snapshots: [
//             "/xport/Screenshot (293).png",
//             "/xport/Screenshot (294).png",
//             "/xport/Screenshot (295).png",
//             "/xport/Screenshot (296).png",
//             "/xport/Screenshot (298).png",
//             "/xport/Screenshot (299).png",
//         ],
//         height: 1900,
//         others: [
//             {
//                 link: "/Projects/2",
//                 thumbnail: "/sai_banner4.png",
//                 logo: "/Skilledu.png",
//                 title: "Skilledu ",
//                 description: `StudyAI.`
//             },
//             {
//                 link: "/Projects/3",
//                 thumbnail: "/Taskly_banner.png",
//                 logo: "/Taskly.png",
//                 title: "Taskly Pro ",
//                 description: `Your Task Manager.`
//             },

//         ],
//     },
//     {
//         title: "Study AI",
//         name: "Skilledu",
//         banner: "/sai_banner4.png",
//         detailBanner: "/Skilledu_banner.png",
//         skills: [
//             "JavaScript",
//             "React.js",
//             "Node.js",
//             "Express.js",
//             "MongoDB",
//             "Tailwind CSS",
//             "CSS3",
//             "REST APIs",
//             "Groq AI Integration",
//         ],
//         description: `Skilledu (StudyAI) is an AI-powered doubt-solving and learning assistance platform designed to help students get instant, accurate answers to academic questions. It provides a clean and student-friendly interface to ask doubts, explore explanations, and enhance conceptual understanding in real time.`,
//         //(Optional) Live link of your project, if hosted
//         link: "sai.skilledu.in",
//         github: null,
//         // Logo Image of your project
//         logo: "/Skilledu.png",
//         features: [
//             "AI-powered doubt-solving for academic questions.",
//             "Instant responses with clear and easy-to-understand explanations.",
//             "Supports multiple subjects and learning topics.",
//             "Clean, distraction-free UI focused on student productivity.",
//             "Responsive design for desktop, tablet, and mobile devices.",
//             "Secure backend with scalable API architecture.",
//             "Designed to support continuous learning and self-study.",
//         ],
//         // Screenshots of your project
//         snapshots: [
//             "/xport/Screenshot (300).png",
//             "/xport/Screenshot (301).png",
//             "/xport/Screenshot (302).png",
//             "/xport/Screenshot (303).png",
//             "/xport/Screenshot (304).png",
//             "/xport/Screenshot (305).png",
//         ],
//         height: 1900,
//         others: [
//             {
//                 link: "/Projects/3",
//                 thumbnail: "/Taskly_banner.png",
//                 logo: "/Taskly.png",
//                 title: "Taskly Pro ",
//                 description: `Your Task Manager.`
//             },
//             {
//                 link: "/Projects/4",
//                 thumbnail: "/bhasha_banner.png",
//                 logo: "/Bhasha_logo.png",
//                 title: "Bhasha Setu ",
//                 description: `Indian Script Transliterator.`
//             },

//         ],
//     },
//     {
//         title: "Your Task Manager",
//         name: "Taskly Pro",
//         banner: "/Taskly_banner2.png",
//         detailBanner: "/Taskly_banner.png", // Different banner for detail page
//         skills: [
//             "JavaScript",
//             "React.js (Vite)",
//             "Tailwind CSS",
//             "HTML5",
//             "CSS3",
//             "LocalStorage",
//             "date-fns",
//         ],
//         description: `Taskly Pro is a modern and responsive task planner that helps users manage, organize, and track daily tasks efficiently. It provides a clean and minimal interface to plan goals, set priorities, manage categories, and analyze productivity — all stored securely on the client side.`,
//         //(Optional) Live link of your project, if hosted
//         link: "taskly-pro-task-manager.vercel.app",
//         github: "https://github.com/Garvit-developer/Taskly_Pro",
//         // Logo Image of your project
//         logo: "/Taskly.png",
//         features: [
//             "Add, edit, delete, and complete tasks effortlessly.",
//             "Organize tasks by categories such as Work, Personal, and Health.",
//             "Filter and search tasks by category, priority, or task name.",
//             "Set due dates with automatic highlighting of overdue tasks.",
//             "Analytics dashboard to visualize productivity and task completion.",
//             "Import, export, and clear task data easily.",
//             "Persistent offline storage using browser localStorage.",
//             "Responsive and visually clean light-themed UI built with Tailwind CSS.",
//         ],
//         // Screenshots of your project
//         snapshots: [
//             "/xport/Screenshot (306).png",
//             "/xport/Screenshot (307).png",
//             "/xport/Screenshot (309).png",
//             "/xport/Screenshot (310).png",
//             "/xport/Screenshot (311).png",
//             "/xport/Screenshot (312).png",
//             "/xport/Screenshot (313).png",
//             "/xport/Screenshot (314).png",
//         ],
//         height: 1900,
//         others: [

//             {
//                 link: "/Projects/4",
//                 thumbnail: "/bhasha_banner.png",
//                 logo: "/Bhasha_logo.png",
//                 title: "Bhasha Setu ",
//                 description: `Indian Script Transliterator.`
//             },
//             {
//                 link: "/Projects/5",
//                 thumbnail: "/Secure_banner.png",
//                 logo: "/Secure_logo.png",
//                 title: "Secure stack ",
//                 description: `Your own Password Manager.`
//             }
//         ],
//     },
//     {
//         title: "Indian Script Transliterator",
//         name: "Bhasha Setu",
//         skills: [
//             "JavaScript",
//             "React.js",
//             "Tailwind CSS",
//             "GROQ API",
//             "Prism"
//         ],
//         description: `Bhaasha Setu is an advanced AI-powered chatbot designed to bridge linguistic barriers across India. It specializes in transliterating text between various Indian scripts, enabling users to read content in their familiar script while preserving the original phonetic sound and language. `,
//         //(Optional) Live link of your project, if hosted
//         link: "bhaasha-setu.vercel.app",
//         github: "https://github.com/Garvit-developer/Bhasha_Setu",
//         // Logo Image of your project
//         logo: "/Bhasha_logo.png",
//         banner: "/bhasha_banner.png",
//         detailBanner: "/bhasha_banner.png",
//         features: [
//             "Developed AI chatbot for accurate transliteration across multiple Indian scripts.",
//             "Preserved original language, pronunciation, and meaning during script conversion.",
//             "Implemented dual output for transliteration and translation.",
//             "Integrated voice input and voice output for hands-free interaction.",
//             "Added multi-language support with dynamic script switching.",
//             "Enabled text and image-based input for transliteration tasks.",
//         ],
//         // Screenshots of your project
//         snapshots: [
//             "/xport/Screenshot (322).png",
//             "/xport/Screenshot (326).png",
//             "/xport/Screenshot (329).png",
//             "/xport/Screenshot (324).png",
//             "/xport/Screenshot (325).png",

//         ],
//         height: 1900,
//         others: [
//             {
//                 link: "/Projects/3",
//                 thumbnail: "/Taskly_banner.png",
//                 logo: "/Taskly.png",
//                 title: "Taskly Pro ",
//                 description: `Your Task Manager.`
//             },
//             {
//                 link: "/Projects/5",
//                 thumbnail: "/Secure_banner.png",
//                 logo: "/Secure_logo.png",
//                 title: "Secure stack ",
//                 description: `Your own Password Manager.`
//             }
//         ],
//     },
//     {
//         title: "Your own Password Manager",
//         name: "Secure stack",
//         banner: "Secure_banner4.png",
//         detailBanner: "/Secure_banner.png",
//         skills: [
//             "React.js",
//             "Tailwind CSS",
//             "Vite",
//             "LocalStorage API",
//             "React Toastify",
//         ],
//         description: "A modern, offline-first password manager built to keep your digital life secure without relying on cloud servers. Your data stays on your device, ensuring maximum privacy.",
//         //(Optional) Live link of your project, if hosted
//         link: "secure-stackk.vercel.app",
//         github: "https://github.com/Garvit-developer/Secure-Stack",
//         // Logo Image of your project
//         logo: "/Secure_logo.png",
//         features: [
//             "Offline-First Storage: All passwords are stored locally in your browser's localStorage for maximum privacy.",
//             "Advanced Password Generator: Create unbreakable passwords with customizable length and character sets.",
//             "Data Portability: Securely export your vault to a JSON file or import from a backup.",
//             "Smart Organization: Categorize passwords and find them instantly with real-time search.",
//             "Security Dashboard: Real-time password strength indicator and vault statistics.",
//         ],
//         // Screenshots of your project
//         snapshots: [
//             "/xport/Screenshot (330).png",
//             "/xport/Screenshot (331).png",
//             "/xport/Screenshot (332).png",
//             "/xport/Screenshot (333).png",
//             "/xport/Screenshot (334).png",
//             "/xport/Screenshot (335).png",
//             "/xport/Screenshot (336).png",
//             "/xport/Screenshot (337).png",
//         ],
//         height: 1900,
//         others: [
//             {
//                 link: "/Projects/3",
//                 thumbnail: "/Taskly_banner.png",
//                 logo: "/Taskly.png",
//                 title: "Taskly Pro ",
//                 description: `Your Task Manager.`
//             },
//             {
//                 link: "/Projects/4",
//                 thumbnail: "/bhasha_banner.png",
//                 logo: "/Bhasha_logo.png",
//                 title: "Bhasha Setu ",
//                 description: `Indian Script Transliterator.`
//             },
//         ],
//     },
//     {
//         title: "Simon Says Game",
//         name: "Simon Memory Challenge",
//         banner: "/Simon_banner.png",
//         skills: [
//             "React.js",
//             "JavaScript",
//             "Tailwind CSS",
//             "Web Audio API",
//             "Recharts",
//             "LocalStorage",
//         ],
//         description: `A modern, interactive memory challenge built with React and Tailwind CSS. Test your focus and memory through increasingly complex color sequences, immersive oscillator-based sound effects, and a data-driven statistics dashboard.`,
//         //(Optional) Live link of your project, if hosted
//         link: null,
//         github: "https://github.com/Garvit-developer/Simon-Says-Game",
//         // Logo Image of your project
//         logo: "/Simon_logo.png",
//         features: [
//             "Interactive gameplay with dynamic sequence generation and real-time user input validation.",
//             "Integrated Data Visualization using Recharts (Bar and Pie charts) to track game history and performance.",
//             "Adaptive Difficulty Levels (Easy, Normal, Hard) that scale the button grid (4 to 8 colors) and flash speeds.",
//             "Immersive Audio Synthesis using Web Audio API to generate unique musical notes for every button press.",
//             "Premium UI/UX featuring glassmorphism, animated gradients, and staggered celebration/confetti effects.",
//             "Persistent Storage for tracking High Scores, Best Levels, and Total Games played across sessions.",
//         ],


//         height: 2200,
//         others: [
//             {
//                 link: "/Projects/1",
//                 thumbnail: "/Geek_banner.png",
//                 logo: "/GeekTheoryIcon.PNG",
//                 title: "Geek Theory Website ",
//                 description: `A modern, IT solutions website for Geek Theory.`
//             },
//             {
//                 link: "/Projects/2",
//                 thumbnail: "/sai_banner4.png",
//                 logo: "/Skilledu.png",
//                 title: "Skilledu ",
//                 description: `StudyAI`
//             },
//             {
//                 link: "/Projects/4",
//                 thumbnail: "/bhasha_banner.png",
//                 logo: "/Bhasha_logo.png",
//                 title: "Bhasha Setu ",
//                 description: `Indian Script Transliterator.`
//             },
//         ],
//     },

// ];



// Replace below project details with your project details. Add all your projects details here as per below object structure, and it will populate it in projects sections.
export const projectsdata = () => [
    {
        title: "IT solutions and software devlopment Website",
        name: "Geek Theory Website",
        banner: "/Banner/Geek_banner2.webp",
        detailBanner: "/Banner/Geek_banner.webp",
        skills: [
            "CSS3",
            "JavaScript",
            "Node.js",
            "React.js",
            "Express",
            "MongoDB",
            "Tailwind CSS",
            "REST APIs",
        ],
        description: `GeekTheo is an IT solutions company that delivers modern, scalable, and performance-driven digital services. The company specializes in web development, application design, and custom business solutions for clients. GeekTheo focuses on building secure, reliable, and user-focused systems using latest technologies. The website represents the company’s services, portfolio, and digital identity.`,
        link: "geektheo.com",
        github: null,
        logo: "/Banner/GeekTheoryIcon.webp",
        features: [
            "Upload and share any file of size upto 1GB of any format like text file, images, videos, application, pdf, excel, ppt and more.",
            "Other important sections like User Profile, Tracking of number of files shared, amount of data transported, authentication and more.",
        ],
        snapshots: [
            "/xport/Screenshot (293)-converted.webp",
            "/xport/Screenshot (294)-converted.webp",
            "/xport/Screenshot (295)-converted.webp",
            "/xport/Screenshot (296)-converted.webp",
            "/xport/Screenshot (298)-converted.webp",
            "/xport/Screenshot (299)-converted.webp",
        ],
        height: 1900,
        others: [
            {
                link: "/Projects/2",
                thumbnail: "/Banner/sai_banner4.webp",
                logo: "/Banner/Skilledu.webp",
                title: "Skilledu ",
                description: `StudyAI.`
            },
            {
                link: "/Projects/3",
                thumbnail: "/Banner/Taskly_banner.webp",
                logo: "/Banner/Taskly.webp",
                title: "Taskly Pro ",
                description: `Your Task Manager.`
            },
        ],
    },
    {
        title: "Study AI",
        name: "Skilledu",
        banner: "/Banner/sai_banner4.webp",
        detailBanner: "/Banner/Skilledu_banner.webp",
        skills: [
            "JavaScript",
            "React.js",
            "Node.js",
            "Express.js",
            "MongoDB",
            "Tailwind CSS",
            "CSS3",
            "REST APIs",
            "Groq AI Integration",
        ],
        description: `Skilledu (StudyAI) is an AI-powered doubt-solving and learning assistance platform designed to help students get instant, accurate answers to academic questions. It provides a clean and student-friendly interface to ask doubts, explore explanations, and enhance conceptual understanding in real time.`,
        link: "sai.skilledu.in",
        github: null,
        logo: "/Banner/Skilledu.webp",
        features: [
            "AI-powered doubt-solving for academic questions.",
            "Instant responses with clear and easy-to-understand explanations.",
            "Supports multiple subjects and learning topics.",
            "Clean, distraction-free UI focused on student productivity.",
            "Responsive design for desktop, tablet, and mobile devices.",
            "Secure backend with scalable API architecture.",
            "Designed to support continuous learning and self-study.",
        ],
        snapshots: [
            "/xport/Screenshot (300)-converted.webp",
            "/xport/Screenshot (301)-converted.webp",
            "/xport/Screenshot (302)-converted.webp",
            "/xport/Screenshot (303)-converted.webp",
            "/xport/Screenshot (304)-converted.webp",
            "/xport/Screenshot (305)-converted.webp",
        ],
        height: 1900,
        others: [
            {
                link: "/Projects/3",
                thumbnail: "/Banner/Taskly_banner.webp",
                logo: "/Banner/Taskly.webp",
                title: "Taskly Pro ",
                description: `Your Task Manager.`
            },
            {
                link: "/Projects/4",
                thumbnail: "/Banner/bhasha_banner.webp",
                logo: "/Banner/Bhasha_logo.webp",
                title: "Bhasha Setu ",
                description: `Indian Script Transliterator.`
            },
        ],
    },
    {
        title: "Your Task Manager",
        name: "Taskly Pro",
        banner: "/Banner/Taskly_banner2.webp",
        detailBanner: "/Banner/Taskly_banner.webp",
        skills: [
            "JavaScript",
            "React.js (Vite)",
            "Tailwind CSS",
            "HTML5",
            "CSS3",
            "LocalStorage",
            "date-fns",
        ],
        description: `Taskly Pro is a modern and responsive task planner that helps users manage, organize, and track daily tasks efficiently. It provides a clean and minimal interface to plan goals, set priorities, manage categories, and analyze productivity — all stored securely on the client side.`,
        link: "taskly-pro-task-manager.vercel.app",
        github: "https://github.com/Garvit-developer/Taskly_Pro",
        logo: "/Banner/Taskly.webp",
        features: [
            "Add, edit, delete, and complete tasks effortlessly.",
            "Organize tasks by categories such as Work, Personal, and Health.",
            "Filter and search tasks by category, priority, or task name.",
            "Set due dates with automatic highlighting of overdue tasks.",
            "Analytics dashboard to visualize productivity and task completion.",
            "Import, export, and clear task data easily.",
            "Persistent offline storage using browser localStorage.",
            "Responsive and visually clean light-themed UI built with Tailwind CSS.",
        ],
        snapshots: [
            "/xport/Screenshot (306)-converted.webp",
            "/xport/Screenshot (307)-converted.webp",
            "/xport/Screenshot (309)-converted.webp",
            "/xport/Screenshot (310)-converted.webp",
            "/xport/Screenshot (311)-converted.webp",
            "/xport/Screenshot (312)-converted.webp",
            "/xport/Screenshot (313)-converted.webp",
            "/xport/Screenshot (314)-converted.webp",
        ],
        height: 1900,
        others: [
            {
                link: "/Projects/4",
                thumbnail: "/Banner/bhasha_banner.webp",
                logo: "/Banner/Bhasha_logo.webp",
                title: "Bhasha Setu ",
                description: `Indian Script Transliterator.`
            },
            {
                link: "/Projects/5",
                thumbnail: "/Banner/Secure_banner.webp",
                logo: "/Banner/Secure_logo.webp",
                title: "Secure stack ",
                description: `Your own Password Manager.`
            }
        ],
    },
    {
        title: "Indian Script Transliterator",
        name: "Bhasha Setu",
        banner: "/Banner/bhasha_banner.webp",
        detailBanner: "/Banner/bhasha_banner.webp",
        skills: [
            "JavaScript",
            "React.js",
            "Tailwind CSS",
            "GROQ API",
            "Prism"
        ],
        description: `Bhaasha Setu is an advanced AI-powered chatbot designed to bridge linguistic barriers across India. It specializes in transliterating text between various Indian scripts, enabling users to read content in their familiar script while preserving the original phonetic sound and language. `,
        link: "bhaasha-setu.vercel.app",
        github: "https://github.com/Garvit-developer/Bhasha_Setu",
        logo: "/Banner/Bhasha_logo.webp",
        features: [
            "Developed AI chatbot for accurate transliteration across multiple Indian scripts.",
            "Preserved original language, pronunciation, and meaning during script conversion.",
            "Implemented dual output for transliteration and translation.",
            "Integrated voice input and voice output for hands-free interaction.",
            "Added multi-language support with dynamic script switching.",
            "Enabled text and image-based input for transliteration tasks.",
        ],
        snapshots: [
            "/xport/Screenshot (322)-converted.webp",
            "/xport/Screenshot (326)-converted.webp",
            "/xport/Screenshot (329)-converted.webp",
            "/xport/Screenshot (324)-converted.webp",
            "/xport/Screenshot (325)-converted.webp",
        ],
        height: 1900,
        others: [
            {
                link: "/Projects/3",
                thumbnail: "/Banner/Taskly_banner.webp",
                logo: "/Banner/Taskly.webp",
                title: "Taskly Pro ",
                description: `Your Task Manager.`
            },
            {
                link: "/Projects/5",
                thumbnail: "/Banner/Secure_banner.webp",
                logo: "/Banner/Secure_logo.webp",
                title: "Secure stack ",
                description: `Your own Password Manager.`
            }
        ],
    },
    {
        title: "Your own Password Manager",
        name: "Secure stack",
        banner: "/Banner/Secure_banner4.webp",
        detailBanner: "/Banner/Secure_banner.webp",
        skills: [
            "React.js",
            "Tailwind CSS",
            "Vite",
            "LocalStorage API",
            "React Toastify",
        ],
        description: "A modern, offline-first password manager built to keep your digital life secure without relying on cloud servers. Your data stays on your device, ensuring maximum privacy.",
        link: "secure-stackk.vercel.app",
        github: "https://github.com/Garvit-developer/Secure-Stack",
        logo: "/Banner/Secure_logo.webp",
        features: [
            "Offline-First Storage: All passwords are stored locally in your browser's localStorage for maximum privacy.",
            "Advanced Password Generator: Create unbreakable passwords with customizable length and character sets.",
            "Data Portability: Securely export your vault to a JSON file or import from a backup.",
            "Smart Organization: Categorize passwords and find them instantly with real-time search.",
            "Security Dashboard: Real-time password strength indicator and vault statistics.",
        ],
        snapshots: [
            "/xport/Screenshot (330)-converted.webp",
            "/xport/Screenshot (331)-converted.webp",
            "/xport/Screenshot (332)-converted.webp",
            "/xport/Screenshot (333)-converted.webp",
            "/xport/Screenshot (334)-converted.webp",
            "/xport/Screenshot (335)-converted.webp",
            "/xport/Screenshot (336)-converted.webp",
            "/xport/Screenshot (337)-converted.webp",
        ],
        height: 1900,
        others: [
            {
                link: "/Projects/3",
                thumbnail: "/Banner/Taskly_banner.webp",
                logo: "/Banner/Taskly.webp",
                title: "Taskly Pro ",
                description: `Your Task Manager.`
            },
            {
                link: "/Projects/4",
                thumbnail: "/Banner/bhasha_banner.webp",
                logo: "/Banner/Bhasha_logo.webp",
                title: "Bhasha Setu ",
                description: `Indian Script Transliterator.`
            },
        ],
    },
    {
        title: "Simon Says Game",
        name: "Simon Memory Challenge",
        banner: "/Banner/Simon_banner.webp",
        skills: [
            "React.js",
            "JavaScript",
            "Tailwind CSS",
            "Web Audio API",
            "Recharts",
            "LocalStorage",
        ],
        description: `A modern, interactive memory challenge built with React and Tailwind CSS. Test your focus and memory through increasingly complex color sequences, immersive oscillator-based sound effects, and a data-driven statistics dashboard.`,
        link: null,
        github: "https://github.com/Garvit-developer/Simon-Says-Game",
        logo: "/Banner/Simon_logo.webp",
        features: [
            "Interactive gameplay with dynamic sequence generation and real-time user input validation.",
            "Integrated Data Visualization using Recharts (Bar and Pie charts) to track game history and performance.",
            "Adaptive Difficulty Levels (Easy, Normal, Hard) that scale the button grid (4 to 8 colors) and flash speeds.",
            "Immersive Audio Synthesis using Web Audio API to generate unique musical notes for every button press.",
            "Premium UI/UX featuring glassmorphism, animated gradients, and staggered celebration/confetti effects.",
            "Persistent Storage for tracking High Scores, Best Levels, and Total Games played across sessions.",
        ],
        height: 2200,
        others: [
            {
                link: "/Projects/1",
                thumbnail: "/Banner/Geek_banner.webp",
                logo: "/Banner/GeekTheoryIcon.webp",
                title: "Geek Theory Website ",
                description: `A modern, IT solutions website for Geek Theory.`
            },
            {
                link: "/Projects/2",
                thumbnail: "/Banner/sai_banner4.webp",
                logo: "/Banner/Skilledu.webp",
                title: "Skilledu ",
                description: `StudyAI`
            },
            {
                link: "/Projects/4",
                thumbnail: "/Banner/bhasha_banner.webp",
                logo: "/Banner/Bhasha_logo.webp",
                title: "Bhasha Setu ",
                description: `Indian Script Transliterator.`
            },
        ],
    },
];
