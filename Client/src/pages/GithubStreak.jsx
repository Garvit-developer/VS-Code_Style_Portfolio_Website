...
const GithubStats = () => {
    ...
    const getColor = (count) => {
        ...
        if (count < 3) return "#033A16";
        if (count < 5) return "#196c2e";
        if (count < 7) return "#56d364";
        return "#39d353";
    };
    ...
    const getMonthLabels = () => {
        ...
        // Filter out the first label if it's too close to the second one (prevent collision)
        if (months.length > 1 && (months[1].index - months[0].index) < 2) {
            months.shift();
        }
                                                ...
                                                {weeks.map((week, wIndex) => (
                                                    <div key={wIndex} className="flex flex-col gap-[3px] ">
                                                        {week.contributionDays.map((day) => (
                                                            ...
                                                            >
                                                                <div className="absolute bottom-full left-1/2 -translate-x-[160px] mb-2 hidden group-hover:block w-max bg-[#6e7681] text-white text-xs rounded py-1 px-2 z-50 shadow-xl pointer-events-none">
